...
There is sometimes (often?) no right answer as to the best format for a particular use case—there are pros and cons to each. (With that said, there is often a clearly _less good_ choice for data storage and sharing—we're looking at you, PDF.) What this means is that in your workflow you will have to deal with data that needs to be extracted from a multitude of systems, and will be available to you in a multitude of formats.
...
* Are the variable types (or classes) for those variables what you anticipated?
  ...
  - If working with labelled variables\index{labelled variables}, did they load as factor type?

* How are missing values\index{missing values} represented?
