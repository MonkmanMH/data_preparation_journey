...
For example, if we want to plot the population by the UA level, an approach would be to identify all those rows that have an Area code that starts with one of the following:

Unitary Authorities (E06), London Boroughs (E09), Counties (E10), and Metropolitan Counties (E11)

For this, we can design a regular expression that 
...
While this approach works for our particular example, it 

### Approach 2 - build a concordance table

The first approach we can take is to create a separate concordance table, and then join that table to our data.
