...
Finally, the {labelled} package [@R-labelled] contains the function `look_for()`, which generates a data dictionary of the dataframe. ^[Joseph Larmarange, ["Generate a data dictionnary and search for variables with look_for()"](http://larmarange.github.io/labelled/articles/look_for.html)]

Here, we read the SPSS version of the {palmerpenguins} file, and generate a data dictionary using the `look_for()` function.

```{r}

penguins_sav <- read_sav(dpjr::dpjr_data("penguins.sav"))
