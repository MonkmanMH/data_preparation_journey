...
For this example, we will use some mock data that is the type a data analyst might encounter when undertaking analysis of a human resource data to support business decision-making. (Remember the "what proportion of staff could potentially retire in the next three years?" example from [\@ref(what-is-data)]?)
...
The one we want for our example is "df_HR_main", which contains the employee ID, name, birth date, and gender of 999 staff.
...
As we saw in the earlier example, the first few rows at the top of the spreadsheet contain the title (now in the variable name row), along with things like the source and the release date. This is important documentation, but it's not part of the data we want for our analysis.

Another of the `read_excel()` arguments to specify the rectangular geometry that contains our data is `skip =`. Here we will use it to skip the first 6 rows.
