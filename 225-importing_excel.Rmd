...
For the UK Census data, we will create two separate concordance tables. We will save these tables as a CSV file, so they are readily available for other applications. 
...
Our next step is to extract the unique values in the area code and area name from the original data in the object `uk_census_pop`.
...
* A variable with lowest-level value, i.e. the values in the source table column B.
