...
There are some approaches to finding problems in the data that can be automated (done programmatically). These are sometimes called "data validation"\index{data validation} methods; they can be applied when the data is collected (for example, in a web-based survey tool) or after the data has been collected. We can think of this as a form of quality control, from quick sanity checks to programmatic testing. [@White_etal_nine_simple_ways]
...
This comes with risks, though. Forcing someone to complete a field might lead to made-up values, which might be worse than a missing field. Imagine forcing someone to report their income—they might enter a "1" or a preposterously very large number, neither of them accurate.
