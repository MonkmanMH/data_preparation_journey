...
Another tool for summary statistics EDA is the package {skimr}[@R-skimr]. The function `skim()` splits the output by variable type. There are no numeric variables in the HR training data; if there were, those would have the same statistics as produced by the `summary()` function, including the mean, standard deviation, and the interquartile range.
...
The `skim()` function, and others in the {skimr} package, can be put into a piped sequence.



The `skim()` output can be streamlined, depending on the data and the questions you are exploring. The first option is to specify the variable types with the `yank()` function:
...
We can also specify the outputs, using the `focus()` function. For our HR data, we might be only interested in the "n_missing" number.
