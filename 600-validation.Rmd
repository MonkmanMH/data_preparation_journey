...
There are some approaches to finding problems in the data that can be automated (done programmatically). These are sometimes called "data validation"\index{data validation} methods; they can be applied when the data is collected (for example, in a web-based survey tool) or after the data has been collected.
...
* All of these have risks, though. Forcing someone to complete a field might lead to made-up values, which might be worse than a missing field. Imagine forcing someone to report their income—they might enter a "1" or a preposterously very large number, neither of them accurate.

For our work here, we will focus on checking and validating data after is has been collected. The analyst can specify parameters prior to it being made available for the next steps. These can be expressed as rules or assumptions, and are based on knowledge of the subject. [@Van_der_Loo_de_Jonge_2018, Chapter 6: Data Validation]
...
df_HR_train <- readxl::read_excel(dpjr::dpjr_data("cr25/cr25_human_resources.xlsx"),
                   sheet = "df_HR_train")
...
The variables that start with `train_` have been stored as character _<chr>_, because of the "NA" string. This variable could be more useful in our analysis if it was instead stored as logical. One option is to mutate the variable types on the existing object in our environment, or (taking the "go as far as possible in the `read_` function" approach) we can reread the contents of the file and apply the variable type definition at that point. The second option also gives us a way to explicitly define as `NA` those variables where it has been coded as a different character. Here `NA` is represented as "NA", but in other data you may encounter "not available", "9999", "-", and host of other possibilities.
...
Not that the variables are in a format we can use, applying R's `summary()` function gives us another way to see the data.
...
A few NA values might be expected in a dataframe like this, and may not have an impact on our analysis. But the `train_qc` variable has nearly 40% of the values missing...there might be a reason for this, and part of the analyst's responsibility is to make note of this, and perhaps further investigate (and record) the reason for this situation. You might start with the data's documentation, and if that doesn't hold an explanation, talking to the people who understand how the data are collected, and then the people in the human resources department...perhaps there's a policy that exempts a cohort from what is otherwise a mandatory course.
...
df_HR_main <- readxl::read_excel(dpjr::dpjr_data("cr25/cr25_human_resources.xlsx"),
                   sheet = "df_HR_main",
...
The first function we will investigate in the package is `check_that()`, using the data in {palmerpenguins}. We know that the data in the package are nice and clean—they have already gone through a validation process.
