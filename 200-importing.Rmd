...
We already know that Excel files can contain a lot of information that is outside the data rectangle. Rather than ignore all that at the start, we will use the function `tidyxl::xlsx_cells()` to read the entire sheet, and we can trim it down to the data we need from there.
...
The "local_format_id" is assigned by {tidyxlr}, and helps us solve our problem. It contains a unique value for every different type of formatting that is used in the sheet.
...
If we examing the "formats" object, we can see that it contains two lists, one called "local" and the other "style". If look inside "local", we see one called "alignment", and within that is "indent"—this is what we're looking for. We can extract that list-within-a-list using the code below:
