...
Here's what the first record look like:
...
| :--- | :---     | :---:  | :---:  |
| SPJ1_TYP | country | 1 | 13 |
| GHJ1DHDI | Health Description Index | 1 | 32 |
| SDJ1GHED | Highest level of post-secondary education attained | 1 | 32 |


You will note in the code below that because the variables occupy only one character, the start and end positions are the same.

```{r jcush_txt_read_vars}

readr::read_fwf(dpjr::dpjr_data("JCUSH.txt"), 
         fwf_cols(
           SPJ1_TYP = c(13, 13), 
           GHJ1DHDI = c(32, 32),
           SDJ1GHED = c(502, 502)
