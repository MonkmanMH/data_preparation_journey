...
PENGUINS EXCEL CODE BOOK

```{r}
penguins_path <- here::here("data", "penguins_coded.xlsx")
...
# alternative approach with {dpjr} package
library(dpjr)
penguins_path <- dpjr_data("penguins_coded.xlsx")
...
The second sheet in the Excel file, "penguins_codebook", contains the values associated with each variable. 

We could add the labels with a coded solution, using the `val_labels()` function from the {labelled} package:
...
# convert to factor with `unlabelled()`
penguins_data %>% 
  unlabelled() %>% 
  group_by(species) %>% 
  tally()
...
We can also read in the values, and apply them programmatically. This will save time and effort, and reduce the risk of typographical errors, particularly if there are many variables with many values to label.

First, we read the contents of the second sheet in the Excel file, containing the codebook:

```{r}

penguins_path <- here::here("data", "penguins_coded.xlsx")

penguins_code <- read_excel(penguins_path, sheet = "penguins_codebook")
