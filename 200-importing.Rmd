...
One thing to notice about this data is the "Area name" field is hierarchical, with a series of sub-totals. The value "England and Wales" contains the total population, which is the sum of England (row x) and Wales (row 360). England is then further subdivided into ^[You can download a map of these regions at ["Local Authority Districts, Counties and Unitary Authorities (April 2021) Map in United Kingdom"](https://geoportal.statistics.gov.uk/documents/ons::local-authority-districts-counties-and-unitary-authorities-april-2021-map-in-united-kingdom--1/about)] into 

The {readxl} package's `read_excel()` function is the one we want to use. The first item is the path to the file. The function's default is to read the first sheet in the Excel file, which is not what we want, so we use the `sheet = ` argument to specify "P01". We will also add the first 
...
The function has a variety of arguments that a
...
But this loses the hierarchy. We will return to this file in the "Cleaning Solutions" chapter.

[MOVE BLOCK BELOW TO CLEANING CHAPTER]

The leading space we see when we view the Excel file in its native software is not created by space characters, but through Excel's "indent" formatting.
...
https://nacnudus.github.io/tidyxl/articles/tidyxl.html

https://stackoverflow.com/questions/54347764/how-to-read-cell-formatting-indentation-information-from-excel-into-r

https://www.youtube.com/watch?v=Q_McYaDV9H0&ab_channel=DuncanGarmonsway


The first thing we need to be aware of is that the {tidyxl} package has some functions that read the first sheet in the file, and won't allow specification of other sheets. But that's ok, because we can write some R code to rearrange the sheets for us.
...
#openXL(uk_census_path)
uk_census_file <- loadWorkbook(uk_census_path)

worksheetOrder(uk_census_file)
names(uk_census_file)

worksheetOrder(uk_census_file) <- c(4, 1, 2, 3, 5:8)
writeData(uk_census_file, 4, 'This is still the "mtcars" worksheet')

worksheetOrder(uk_census_file)

# now write the new version
saveWorkbook(uk_census_file, "data/census2021_P01.xlsx", overwrite = TRUE)
openXL("data/census2021_P01.xlsx")
...
test_path <- "data/sed17-sr-tab012.xlsx"

test_cells <- xlsx_cells(test_path)
...
test_format <- xlsx_formats(test_path)

indent <- test_format$local$alignment$indent
...
test_cells %>% 
  ...
  behead_if(direction = "WNW", name = "field3") %>% 
  dplyr::filter(row == 8)
...
#uk_census_path <- "data/census2021firstresultsenglandwales1.xlsx"
uk_census_path <- "data/census2021_P01_manual.xlsx"
#uk_census_path <- "data/nonsense.xlsx"
...
```{r}
#uk_census_col02 <- uk_census %>% 
#  filter(row >= 7) %>% 
#  filter(col == 2)
#
#uk_census_col02
#
#glimpse(uk_census_col02) 
...
select(uk_census, row, col, data_type, character, numeric, local_format_id)
...
uk_census %>% 
  filter(col != 1) %>% 
  dplyr::filter((row >= 2)) %>% 
  behead_if(indent[local_format_id] == 0, direction = "WNW", name = "field0") %>% 
  ...
  behead_if(direction = "WNW", name = "field4") %>% 
  dplyr::filter(row < 15)
```
