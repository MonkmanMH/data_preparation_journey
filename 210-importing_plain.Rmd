...
Base R [@R-base] has a number of functions to read CSV, TSV, and fixed-width files. In the following examples, we will use versions of the data in the {palmerpenguins} data [@Horst_Presmanes_Hill_Gorman_2022; @R-palmerpenguins]

For example, `read.csv()` is a base R function to read CSV files.

In the code chunks below, an R object `penguins_path` is created that contains the text string with the location of the file "penguins.csv". In the first example, the file is located in a directory called "data" that is in our RStudio project folder. The {here} package will generate the full path, with slash or backslash separators (depending on your computer's operating system), starting at the location of the RStudio project.
...
In this second example, the file has been loaded to our computer using the {dpjr} package, and uses the `dpjr_data()` function from that package to access the file (no matter its location on our computer).
...
This chunk creates a string object that contains the path name, and then uses the function `read_csv()` to create an object called `penguins_data`, using the `penguins_path` object, which contains the file path to the the CSV file "penguins.csv".

* Note: this is the {readr} function `read` _underscore_ `csv`, not base R's `read` _dot_ `csv`.
