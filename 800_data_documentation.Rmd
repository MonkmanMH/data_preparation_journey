...
The documentation of the data sources, methods, and workflow is vital for reproducibility\index{reproducibility}.
...
It's also essential if you intend to share your data [@White_etal_nine_simple_ways]. The documentation we create provides metadata (which we will see more of in the data dictionary section below), 


## Metadata

[@White_etal_nine_simple_ways]

* see additional references under "Metadata"
...
As well, things change (the data, the software and hardware we use, the motivations for our work) and capturing the details about those things helps us make any necessary changes that might be required at a later date.



There doesn't seem to be a single-best how-to instruction on "documentation" in the context of data science. The term appears in the periphery of some of the textbooks and manuals on how to do data science, and there are some journal articles that address documentation in the context of reproducible research. It seems what advice exists tends to be prescriptive and at a high level, and not overly descriptive of what makes good documentation.
...
Similar sentiments are expressed in a blog post by Randy Au, "Let's Get Intentional About Documentation". ^[["Let's Get Intentional About Documentation"](https://counting.substack.com/p/lets-get-intentional-about-documentation), 2020-09-29] While the article's key idea is in the title, Au presents the case that a better term might be "recordkeeping".
...
"Let us change our traditional attitude to the construction of programs: Instead of imagining that our main task is to instruct a computer what to do, let us concentrate rather on explaining to human beings what we want a computer to do." —Donald Knuth [citation needed!]
...
So what are the elements of effective recordkeeping? 

These two quotes capture the key concepts:
...
* Details about the analysis and code:
  ...
  - The aim is to help future readers, including yourself, understand the code. 
...
Files, data tables, variables, and the values within those variables are all things that we might want to name (if we are the creator) or rename (if we are the editor).

[@Hermans_programmers_brain_2021]
...
Also see [@R-labelled] http://larmarange.github.io/labelled/articles/look_for.html
...
Here are a couple of articles with good ideas about how to structure your projects:
