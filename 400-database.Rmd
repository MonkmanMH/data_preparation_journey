...
con_nycf <- DBI::dbConnect(RSQLite::SQLite(), "data/nycflights13_sql.sqlite")
...
In addition to writing native R code, we can embed SQL inside R code. In the example below, an R chunk in the R Markdown document has R code that uses the `dbSendQuery()` function. Inside this function, we first name the connection we are using, and then inside the quotation marks write SQL code: `"SELECT * FROM flights WHERE dest = 'SEA'"`. This query instruction gets saved as the object `SEA_sql`.
...
SEA_sql <- dbSendQuery(con_nycf, "SELECT * FROM flights WHERE dest = 'SEA'")
dbFetch(SEA_sql)
...
Once this has been done, our R Markdown document can incorporate native SQL queries into the workflow. Note that in our SQL chunk, we specify the connection. As we continue to work through this example, this is the `con_nycf` object created earlier.
...
The joins are named using terms similar to those you are familiar with from {dplyr}. (More accurately, the join functions in {dplyr} are explicitly ) This left join will return all of the records from the `flights` table, and the variables from `airlines` where there is a match.
...
After running the `library()` function, the results are automatically generated.

The 30 Major League Baseball teams play in cities in the United States and Canada (only Toronto!), but draw the best players from around the world. In this example the code creates a summary table to show the country of birth of the player who batted in the 1990 season.
...
Lahman::Batting %>% 
  ## filter
  filter(yearID == "1990") %>% 
  ## join to People (where the birth country is recorded)
