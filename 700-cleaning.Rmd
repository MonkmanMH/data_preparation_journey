...
One of the {stringr} functions is `str_detect()`, tests for the presence of a pattern, and returns a logical value (true or false). Here, we test the list of four postal codes using the `postalcode` object that contains the regular expression for Canadian postal codes.

```{r str_detect_postalcode}

str_detect(string = pc1, pattern = postalcode)

```

As we see, the last character string—which contains an extra digit at the end—returns a "FALSE" value. 
