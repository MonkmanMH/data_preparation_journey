...
Our first approach will be to simply drop those years that are missing a value. This might help with some calculations (for example, putting a `na.rm = TRUE` argument in a `mean()` function is doing just that).
...
```{r}
ggplot(naics721_sal, aes(x = ref_year, y = value_new)) +
  geom_line() 
```
