...
The benefits of this format are many, not the least of which is that there are R tools built for us to make our work straight-forward. We will work with {lubridate}, part of the tidyverse.


### Building a date-time field
...
In the {nycflights13} table, the date and time variables are stored as separate variables. They can be combined using these two functions:
...
The second problematic way dates are stored are in a variety of often-ambibuous sequences. This might included text strings for the month (for example "Jan", "January", or "Janvier" rather than "01"), two-digit years (does "12" refer to 2012 or 1912?), and ambiguous ordering of months and days (does "12" refer to December or the twelfth day of the month?)

If we know the way the data is stored, we can use {lubridate} functions to process the data into YYYY-MM-DD.
...
A third manner in which dates can be stored is with a truncated representation. For example, to total sales for the entire month might be recorded as a single value associated with "Jan 2017" or "2017-01".
...
Some useful regex matching functions:
...
"If your regular expression gets overly complicated, try breaking it up into smaller pieces, giving each piece a name, and then combining the pieces with logical operations." —Wickham and Grolemund, _R for Data Science_, ["Detect matches"](https://r4ds.had.co.nz/strings.html#detect-matches)
...
Below, we split an address based on comma location.
...
step 1. remove space or hyphen

* `gsub()` is a function in {base} R
...
* alternative, using `stringr::str_replace()`  https://stringr.tidyverse.org/reference/str_replace.html

* or (preferable) `stringr::str_remove()` https://stringr.tidyverse.org/reference/str_remove.html


```{r}

str_replace(string = pc1, pattern = "-", replacement = "")

str_remove(string = pc1, pattern = "-")

# a regex that removes either space or hyphen
str_remove(string = pc1, pattern = "[ -]")
...
If this was in a data frame (or tibble), these functions can be added to a `mutate()`...

```{r}
# convert string to tibble
pc1_tbl <- as_tibble(pc1)

# mutate new values--note that creating three separate variables is to allow for comparisons as the values change from one step to the next
pc1_tbl %>%
  mutate(value_2 = str_remove(value, "[ -]")) %>%   # remove space or hyphen
  mutate(value_3 = str_to_upper(value_2))           # change to upper case
...
#### Labour Force Survey: remove NAICS code

https://regex101.com/r/jOP4fq/1
