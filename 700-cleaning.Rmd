...
Here are two separate lines to achieve this, one for a space, and one for a hyphen, that uses the `gsub()` function in {base} R.  
...
If this was in a data frame, these functions can be added to a `mutate()`.
...
Note that `str_flatten` has a path to the same destination, but we need to specify the separator:
...
bls_employment <- read_csv("us_bls_employment_2010-2020.csv")
...
The "Series ID" variable string starts with "CEU", then the first 2 digits are the high-level "supersector", of which there are three: primary industries, goods-producing industries, and service-producing industries.

Mining, Quarrying, and Oil and Gas Extraction (NAICS 21) is a sector within the goods-producing industries, with the NAICS code number 21. This is represented at digits 6 and 7 of the string.

Mining (NAICS 212) is a subsector, comprised of three industries (at the four-digit level).


The hierarchical structure of the data means that there is double-counting: the sum of coal, metal ore, and non-metalic mineral industry rows is reported as the employment in Mining (except Oil and Gas).
...
* extract the rows for Mining, Quarrying, and Oil and Gas Extraction (NAICS 21), but only at the 3 digit level.
...
Let's add the digits 1 to 9 to the front of the second filter:
