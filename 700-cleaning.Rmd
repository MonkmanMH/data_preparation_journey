...
We will use the {tidyr} function `separate()` to separate it into two variables. 
...
There is also an option in `fastDummies::dummy_cols()` to `remove_most_frequent_dummy = TRUE`. In our discrimination data, there are more observations coded as "male", so the variable `gender_male` is not included.
...
naics721_sal %>% 
  mutate(down = VALUE) %>% 
  tidyr::fill(VALUE, .direction = "down")  # default direction is down

naics721_sal %>% 
  mutate(up = VALUE) %>% 
  tidyr::fill(VALUE, .direction = "up")  # default direction is down
...
Missing values in time series models can sometimes be problematic, so there's a need to fill in the missing values.


#### {forecast} fill functions

The {forecast} package has time series oriented imputation fill functions

See:

* Hyndman and Athanasopoulos, _Forecasting: Principles and Practice_ (2nd ed), https://otexts.com/fpp2/missing-outliers.html
...
Use `na.interp()` function from {forecast}

```{r}
naics721_sal %>% 
  mutate(val_new = forecast::na.interp(VALUE))
```
