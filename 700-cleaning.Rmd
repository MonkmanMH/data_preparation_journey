...
Another of the {stringr} function, `str_detect()`, tests for the presence of a pattern and returns a logical value (true or false). Here, we test the list of four postal codes using the `postalcode` object that contains the regular expression for Canadian postal codes.
...
More efficiently, we can use a regular expression with both a space and a hyphen inside the square brackets, which has the result of replacing _either_ a space or a hyphen.
...
In Chapter \@ref(importing-stats) we imported statistical software packages such as SPSS, SAS, or Stata and saw the benefits, in certain circumstances, of having labelled variables. These are categorical variables, where there are a limited number of categories. 
