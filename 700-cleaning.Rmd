...
The first step will be to remove the space hyphen, using `stringr::str_replace()` where the replacement string is "". 
...
A more concise way to do this would be to use the `stringr::str_remove()` function. As well, we can use a regular expression with both a space and a hyphen inside the square brackets, which has the result of replacing _either_ a space or a hyphen.
