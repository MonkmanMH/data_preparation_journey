...
Pull names from list

Result is vector with one element
...
Split into 4 separate elements

* this returns nested list
...
In order to have the structure we need, we apply the `unlist()` function:
...
penguin_variable_names <- unlist(str_split(penguin_variable_vector, "\\s+"))
penguin_variable_names
...
# 2. loop
# Because the first element is the titles, we want the length to be one shorter
...
bcf_table_3 <- bcf_table_2 |> 
  ...
  mutate(passengers_fy2021 = as.numeric(passengers_fy2021),
         ...
         passenger_tariff_revenue_fy2021 = as.numeric(passenger_tariff_revenue_fy2021),
         passenger_tariff_revenue_fy2020 = as.numeric(passenger_tariff_revenue_fy2020))
...
bcf_table_3 |> 
  group_by(route_type) |> 
  summarise(total_fy2021 = sum(passengers_fy2021),
            total_fy2020 = sum(passengers_fy2020)) |> 
  mutate(pct_change = (total_fy2021 - total_fy2020) / total_fy2020 * 100)

```


The COVID-19 pandemic had an _enormous_ impact on travel. The fiscal year 2021 covers the period April 1, 2020 through March 31, 2021...starting almost exactly as the responsible goverments imposed strict limitations to travel. Even though the restrictions started to lift later in the 12-month period, the travel on the major routes was down by nearly half.
