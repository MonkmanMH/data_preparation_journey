...
4. Explore and validate the data, assessing the structure, looking for missing values\index{missing values} and other "dirty" elements.
...
* The use of a default (or sentinel) value\index{sentinel value}, inserted as a placeholder for unknown or missing values in place of a missing\index{missing values} or "unknown" value. This is sometimes part of the data collection or database software, leading to values that should have been "unknown" being entered as the default. This can be the consequence of a data entry validation\index{data validation} process that requires an entry (that is, data entry cannot continue until the field has an entry) or an entry in a particular format, such as a date. In these cases, the sentinel value can be entered instead of an explicit "NA".
...
A preferable structure would be to have a variable "year" and another "population". This tidy\index{tidy data} structure also permits other variables measured during those years to be incorporated, rather than displayed in a separate table.
...
We will see some of these challenges and solutions in the next few chapters, as part of the data import process. We will also return to explicitly address these challenges in Chapters \@ref(validation) and \@ref(cleaning).
